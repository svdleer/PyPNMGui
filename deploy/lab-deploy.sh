...
PYPNM_REPO="${PYPNM_SRC:-/home/svdleer/docker/PyPNM}"
...
full_deploy() {
    ...
    ssh "${REMOTE_SERVER}" << 'ENDSSH'
        ...
        cd ${PYPNM_SRC:-/home/svdleer/docker/PyPNM}/
        git fetch origin && git reset --hard origin/main
        # Replace settings/ dir with symlink to /app/config so the entrypoint
        # skips rm-rf (which fails because bind-mount parent is owned by svdleer)
        rm -rf ${PYPNM_SRC:-/home/svdleer/docker/PyPNM}/src/pypnm/settings
        ln -sfn /app/config ${PYPNM_SRC:-/home/svdleer/docker/PyPNM}/src/pypnm/settings
        ...
        cd ${AGENT_SRC:-/home/svdleer/docker/pyPNMAgent}/
        git fetch origin && git reset --hard origin/main
...
quick_deploy() {
    ...
    ssh "${REMOTE_SERVER}" << 'ENDSSH'
        ...
        echo "[INFO] Pulling PyPNM (API backend)..."
        cd ${PYPNM_SRC:-/home/svdleer/docker/PyPNM}/
        git fetch origin && git reset --hard origin/main
        rm -rf ${PYPNM_SRC:-/home/svdleer/docker/PyPNM}/src/pypnm/settings
        ln -sfn /app/config ${PYPNM_SRC:-/home/svdleer/docker/PyPNM}/src/pypnm/settings
