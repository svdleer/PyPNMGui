...
@topology_bp.route('/api/topology/fiber-nodes/scan-targets', methods=['POST'])
def topology_fiber_node_scan_targets_api():
    """Proxy exact technical FiberNode scan-target resolution to PyPNM."""
    ...
    try:
        ...
        if not isinstance(response_payload, dict):
            response_payload = {
                'status': 'error',
                'message': 'invalid scan-target response from PyPNM API',
            }
