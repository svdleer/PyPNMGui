...
class CMTSProvider:
    ...
    def get_cmts_by_hostname(cls, hostname: str) -> Optional[Dict[str, Any]]:
        ...
        for cmts in cmts_list:
            host = str(cmts.get('HostName', '')).strip().lower()
            alias = str(cmts.get('Alias', '')).strip().lower()
            if host == target or alias == target:
                return cmts
        return None

    @classmethod
    def get_cmts_by_ip(cls, ip: str) -> Optional[Dict[str, Any]]:
        """Get a specific CMTS by IP address.

        Matches across common appdb fields (management/device IP variants),
        not only `IPAddress`.
        """
        ...
        def _candidate_ips(rec: Dict[str, Any]) -> set[str]:
            keys = {
                'IPAddress', 'ip', 'ip_address',
                'ManagementIP', 'MgmtIP', 'management_ip', 'mgmt_ip',
                'DeviceIP', 'device_ip',
            }
            vals = set()
            for k in keys:
            ...
            # Also include any field name containing 'ip' to support API schema drift.
            for k, v in rec.items():
                if 'ip' in str(k).lower() and v is not None:
                    s = str(v).strip()
                    if s:
                        vals.add(s)
            return vals
        ...
        cmts_list = cls.get_all_cmts()
        for cmts in cmts_list:
            if target in _candidate_ips(cmts):
                return cmts
        return None
