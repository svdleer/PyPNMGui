...
def configure_utsc():
    """UTSC is already configured via GUI - skip configuration."""
    print("Using existing UTSC configuration (configured via GUI)")
    print("Analyzing existing file timing patterns...")
...
def watch_files(duration=60, retrigger_interval=1, freerun_mode=True):
    """Watch for UTSC files, buffer bursts, and stream evenly."""
    print(f"\nWatching for files (duration={duration}s)...")
    if freerun_mode:
        print("Running in FREERUN mode - UTSC generates files continuously")
        print("NOT triggering manually - monitoring natural file generation")
    else:
        print("Running in TRIGGER mode - will manually trigger UTSC")
