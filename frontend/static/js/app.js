        ...
        async _reconcileTopologyPhysicalFiberNode({ initializeSelection = false } = {}) {
            ...
            const rows = data.records.filter(record => {
                ...
                return {
                    ...
                    // PyPNM's exact topology record is authoritative for
                    // topology location and hierarchy. Search rows are only a
                    // fallback retained for non-exact reconciliation responses.
                    lat: expected?.lat ?? current.lat ?? searchRow.lat ?? null,
                    lon: expected?.lon ?? current.lon ?? searchRow.lon ?? null,
                    topology_expected: expected,
                    topology_fiber_node: expected?.fibernode || searchRow.topology_fiber_node || '',
                    linked_node_id: expected?.linked_node_id || searchRow.linked_node_id || '',
                    topology_node_id: expected?.topology_node_id || searchRow.topology_node_id || '',
                    topology_group_amplifier: expected?.topology_group_amplifier || searchRow.topology_group_amplifier || '',
                    topology_end_amplifier: expected?.topology_end_amplifier || searchRow.topology_end_amplifier || '',
                    topology_tap: expected?.topology_tap || searchRow.topology_tap || '',
                    topology_path: expected?.hierarchy_path || searchRow.topology_path || '',
